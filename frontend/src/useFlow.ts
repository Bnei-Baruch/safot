...
// import { createNewDictionary } from './services/dictionary.service';
import { buildPromptString/*, createPromptRule*/ } from './services/rule.service';
...
const buildAndSaveSegments = async (
  ...
  const ret = await postSegments(segments);
  console.log('post ret segments:', ret);
  return ret;
};
...
export function useFlow() {
  ...
  const translateSegments = useCallback(async (originalSegments: Segment[], translatedSourceId: number, sourceLang: string, targetLang: string)
    ...
    try {
      ...
      const { promptText } = buildPromptString({
          promptKey,
          sourceLanguage: sourceLang,
          targetLanguage: targetLang,
      });

      const { translated_paragraphs, properties } =
          await translateParagraphs(originalSegments.map((segment) => segment.text), sourceLang, targetLang,  promptText);
